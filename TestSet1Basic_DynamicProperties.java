...
import developer.DynamicProperty;
import developer.IDynamicProperty;
...
public class TestSet1Basic_DynamicProperties {
	    ...
	    public void canBeConstructedWithInitialValue() {
    		IDynamicProperty<Integer> p = DynamicProperty.create(42);
	    ...
	    public void nothingImmediatelyHappensWhenSubscribed() {
	    	IDynamicProperty<String> p = DynamicProperty.create("Forty Two");
	        p.subscribe(new IObserver<String>() { @Override public void observe(String value) { Assert.fail(); } });
	    }

	    @Test
	    public void subscriptionReceivesNotificationsWhenValueChanges() {
	    	final IDynamicProperty<Integer> p = DynamicProperty.create(42);
	        final Collection<Integer> notifications = new ArrayList<Integer>();
	
	        // add each notification to our notifications list.
	        p.subscribe(new IObserver<Integer>() { @Override public void observe(Integer value) { notifications.add(value); } });
	    ...
	    public void subscriptionNotNotifiedAfterItIsDisposed() throws IOException {
	        ...
	        // add each notification to our notifications list.
	        Closeable sub = p.subscribe(new IObserver<Integer>() { @Override public void observe(Integer value) { notifications.add(value); } });
	    ...
	    {
	        final IDynamicProperty<Integer> p = DynamicProperty.create(0);
	        ...
	        subscriptions[0] = p.subscribe(new IObserver<Integer>() { @Override public void observe(Integer value) { notifications.get(0).add(value); } });
	        p.setValue(1);
	        expected.get(0).add(1);

	        subscriptions[1] = p.subscribe(new IObserver<Integer>() { @Override public void observe(Integer value) { notifications.get(1).add(value); } });
	        p.setValue(2);
	        expected.get(0).add(2);
	        expected.get(1).add(2);

	        subscriptions[2] = p.subscribe(new IObserver<Integer>() { @Override public void observe(Integer value) { notifications.get(2).add(value); } });
	        p.setValue(3);
